...
public class BossMonster : MonoBehaviour, IEnemy
{
  ...
  [Header("Audio")]
  [SerializeField] private AudioClip ElectricAudio;
  [SerializeField] private AudioClip SniperAudio;
  [SerializeField] private AudioClip FlameAudio;
  ...
  private IEnumerator BossAttackCycle()
  {
    ...
    while (IsAlive)
    {
      ...
      {
        case 0:
          audioSource.PlayOneShot(ElectricAudio);
        ...
        case 3:
          for (int i = 0; i < 4; i++)
          {
            audioSource.PlayOneShot(SniperAudio);
            Shoot(sniperPrefab);
