...
public class FlameThrower : MonoBehaviour, IEnemy
{
  ...
  [SerializeField] private float fireRotation = 40.0f;
  [SerializeField] private float fireRangeY = 0.5f;
  ...
  void Update()
  {
    ...
    if (!intro && Vector3.Distance(transform.position, Target.transform.position) <= 10.0f)
    {
    ...
    {
      Debug.Log("distance too far : " + Vector3.Distance(transform.position, Target.transform.position));
    }
  }
  ...
  public void Shoot(GameObject bullet)
  {
    if (bullet != null && firePoint != null)
    {
      ...
      GameObject missile = Instantiate(
          bullet,
          spawnPosition,
          Quaternion.Euler(0, 0, Random.Range(-fireRotation, fireRotation)));
    }
    else if (bullet == null)
    {
      Debug.LogError("FlameThrower.cs : missile game object not initiated");
    }
    else
    {
      Debug.LogError("FlameThrower.cs : firePoint is null");
    }
  }
